...
> **문서 기준: 2026-09-12 · 이번 세션의 최종 정리**  
> **문서 상태: 구현 기준선과 검증 계획. 구현·배포·성능 검증 완료 보고서가 아니다.**  
> 다음 실행 단위는 **§16의 fixture 기반 PR1**이다. 장기 기능이나 조건부 후보를 PR1에 모두 구현하지 않는다.
...
A/B 테스트와 실제 지표 개선 경험은 다른 프로젝트로 보완할 수 있다. C'mon Yo!에는 제품과 장애를 설명할 최소 관측을 둔다.
...
Supabase는 Auth와 관리형 PostgreSQL로 사용한다. 제품 테이블 접근은 서버의 PostgreSQL adapter로 모은다. runtime role은 최소 권한으로 두고 migration/DDL 권한과 분리한다. 제품 테이블의 Data API 노출을 차단하며, 사용하지 않는 Data API의 비활성화 여부를 실제 설정에서 확인한다. Supabase 공식 문서는 Data API 접근 권한·schema·RLS 경계를 설명한다. [T4]
...
인증·WebView 동일 identity spike를 먼저 수행하고 결과로 방식을 채택·수정·기각한다. Browser·Native의 모임 생성/참여/취소, 권한, last-seat concurrency, 중복 요청을 검증한다. 개인 응답 cache와 계정 전환도 테스트한다.

공유 HTML cache를 켜기 위한 공개/개인 응답 분리 여부는 별도 확인한다. 세션 연결이 작동했다고 cache 안전성까지 통과한 것으로 보지 않는다.

### PR4 — 현장 체크인과 수동 확인

순수 정책 테스트, Geolocation/CoreLocation, 서버 판정, 권한 거부·부정확한 위치 fallback, 주최자 확인, reconciliation을 구현한다. 두 상태축, 취소된 모임, 경계 시각, 중복 요청, raw 위치 redaction을 검증한다.

### PR5 — 커뮤니티와 실제 혼합 UX

donor를 선별 이식한다. 최초 댓글 migration 전에 저장 모델을 비교·기록한다. 웹과 Native의 본문·피드·입력 중 무엇을 공유할지 §5의 경계 명세를 작성하고 실제 화면을 구현한다.

본문 공유, 전체 웹 흐름, Native 입력+웹 목록 등 후보를 기능 이익과 비용으로 평가한다. 모두 만들어 비교하는 것을 의무화하지 않는다. 채택한 경계에서 키보드·작성 보존·복귀·사진 취소·인증 실패를 테스트한다.

### PR6 — 독립 운동량 기능

HealthKit 읽기·운동량 표시를 단독 사용자 흐름으로 완성한다. 모임 참여가 없어도 동작하고 데이터 없음이 미출석/운동량 0으로 확정되지 않아야 한다. 계정 전환·권한 변화·부분 결과와 공개 공유 경계를 확인한다.

PR6 분리는 HealthKit을 축소하려는 것이 아니라 커뮤니티와 서로 독립적으로 검증할 수 있게 한 작업 단위다. API/클라이언트 기반이 준비되면 커뮤니티 작업과 순서를 조정할 수 있다.
...
| 인증 교환 | 동일 identity·보안 요구 고정 | PR3 spike |
| 댓글 DB | 두 요구와 변경 비용 비교 | 최초 댓글 migration 전 |
| 공공데이터 identity | 추측 금지 | PR2 실제 샘플 확인 |
| 체크인 임계값 | pure policy와 version | PR4 경계 테스트·현장 확인 |
