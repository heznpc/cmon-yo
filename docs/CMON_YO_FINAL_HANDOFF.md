...
**첫 시설 화면의 약속:** 공공자료에 등록된 공원 위치와 운동시설 목록을 보여준다. 개별 기구 좌표·실시간 사용 여부·현장 안전을 보장하지 않는다. 최초 지역은 실제 표본을 확보한 무안군이며 선택 가능한 동네와 수집 범위를 일치시킨다. 현재 시설 탐색은 실제 지도에 공원 좌표를 핀으로 표시하고 선택한 시설의 상세로 이동한다. Web은 MapLibre·OpenFreeMap, Native는 MapKit을 사용한다. 지역 확대는 별도의 원본·수집 범위를 확인한다.
...
| 시설 지도 | Web MapLibre·OpenFreeMap / Native MapKit | 별도 API 키 없이 공개 시설 좌표 표시. 외부 배경 연결·핀 선택·상세 이동 검증 |
| 원격 Push·외부 관측 | 현재 핵심 흐름의 선행조건 아님 | 채택 시 APNs/서비스 인증, 관측 수집 주소 등을 그 기능의 설정 목록에 추가 |
...
Web 우선 후속 구현은 [소통·내 활동·현장 확인 수용조건](web-activity-acceptance.md)을 따릅니다. 게시글·댓글·신고/차단·개인 활동·현장 요청/확정을 실제 DB에 연결했으며, 아래 기존 Native/공식 외부 연결의 상태와 구분합니다.

**Web·iOS 로그인과 실제 모임 생성·주최 수정/취소·참여/취소·내 모임을 구현하고 로컬 DB·HTTP·브라우저·Simulator에서 실행했다. Native 소통과 공식 인증 공급자 연결은 후속이다.** 현재 동작과 남은 사용자 흐름을 구분한다.
...
승인된 기능 범위는 단계마다 재승인을 기다리지 않고 구현·검증·수정을 이어간다. 외부 키·공식 실연결 검증이 남아도 로컬 DB·시험 공급자로 독립 기능 개발을 계속한다. 각 PR의 완료 판정과 다음 기능의 개발 진행을 구분한다. 기본 UI의 기능·접근성은 구현 중 확인하고, 최종 브랜드 스타일은 결정 후 별도 스타일 PR에서 Web·Native에 묶어 적용한다. 이 원칙은 PR1 이후에도 동일하다.
...
현재 `docs/pr2-acceptance.md`를 기준으로 구현·실행 증거가 있으며, 인증된 기상청 성공 응답의 값·발표/대상 시각·격자 대조가 남아 있다. 이 항목을 확인하기 전에는 draft를 유지한다. 전국 ingest와 날씨 공급자 교체로 범위를 확대하지 않는다. 시설 지도 탐색은 후속 사용자 흐름으로 추가됐다.
