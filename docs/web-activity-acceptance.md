...
## 실제 GPS 현장 검증 절차 — 2026-09-14

연결된 iPhone 16을 감지했습니다. 이 사실은 현장 방문·브라우저 위치 권한·실제 GPS 성공 증거가 아닙니다. 공개 HTTPS 주소와 현장 방문 가능 여부가 확인되지 않아 실제 현장 검증은 아직 수행하지 않았습니다. [HTTPS 준비와 로컬 검사](../deploy/README.md)를 먼저 구분합니다.

실제 HTTPS 사이트의 Safari에서 이메일 로그인 → 해당 시설의 실제 시험 약속에 참여 → 허용 시간에 시설 방문 → `현재 위치로 확인`을 누르는 흐름을 수행합니다. 현장 참여자가 위치 권한을 직접 허용하고 결과를 확인합니다. 위치 거부 후 사유 입력·수동 요청, 주최자의 다른 계정 승인도 같은 약속에서 확인합니다. 실제 방문 없이 좌표를 주입한 검사는 이 기록에 합산하지 않습니다.

기록에는 날짜, 기종/iOS/Safari, 공개 시설 ID, 현장 안/밖의 조건, 위치 권한 상태, 화면 결과와 서버 오류 code, 수동 요청과 승인 여부만 남깁니다. 개인 계정 ID·좌표·이동 경로·위치가 포함된 스크린샷은 공개하지 않습니다. 시설 밖에서 거절되는지, 부정확한 위치에서 재시도가 가능한지 실제로 확인하지 않았다면 각각 미실행으로 남깁니다. 현장 표본 없이 개발 정책의 200m·100m 임계값을 운영 적합성이 검증된 값으로 바꾸지 않습니다.
