...
## HTTPS 프록시·지속 부하 추가 실행 — 2026-09-14

관련 이슈 [#10](https://github.com/heznpc/cmon-yo/issues/10), 기준 `409a235` 위의 변경 작업 트리입니다. [실행 설정·재현 명령](../deploy/README.md)을 따릅니다. Caddy 2.11.4 공식 macOS arm64 archive의 SHA-512를 대조했습니다. 시스템 신뢰 저장소를 바꾸지 않고 명시적 시험 CA로 TLS를 검증했습니다. 등록된 원격 deployment·environment·서비스 주소는 발견하지 못했으며 아래는 공개 배포 결과가 아닙니다.

`CADDY_BIN=<검증한 바이너리> CMON_LOAD=1 npm run with-env -- qa:https`가 통과했습니다. HTTP/1.1 identity/gzip와 HTTP/2 gzip의 실제 시설 본문→공급자 해제→날씨 도착, 한 소비자 취소/공유 유지, 마지막 소비자의 upstream 취소, timeout/복구, Secure cookie·두 계정·Origin·cache·404·제어 경로 차단의 **8개 시나리오**를 확인했습니다. CA를 주지 않은 TLS 요청은 거절됐습니다. 이는 실제 브라우저의 paint/LCP·인증서 신뢰·배포망 검사가 아닌 TLS 전송 관측입니다.

추가 변경 뒤 `npm run with-env -- check`는 lint·TypeScript·**84개 테스트(skip 0)**·production build·server-only 경계를 통과했고 `AUTH_SECRET= npm run with-env -- test:web`는 dev/prod 브라우저 **34개**를 통과했습니다. 신규 proxy 검사는 기본 신뢰 거부·loopback의 명시적 신뢰·비허용 peer의 위조 전달 헤더 거부를 확인합니다. CI에는 checksum을 대조한 Linux Caddy로 같은 전송 검사를 추가하며, 부하 수치는 CI에서 재생성하지 않습니다.

환경: Apple M4, 논리 CPU 10개, RAM 16GiB, macOS 27, Node 22.22.3, PostgreSQL 17.11. 부하 발생기·Caddy·Node·DB가 같은 Mac이고 네트워크/CPU 제한은 없습니다. 시작 load average(1/5/15분)는 2.79/3.30/3.12입니다. 시설 21·모임 40·게시글 2,000·댓글 8,000·시험 계정 2개를 사용했습니다. 8개 SSR/API/개인 조회 경로를 순환하는 닫힌 루프이며, worker당 요청 1개, 각 조건 warm-up 1초 제외 후 5초×3회입니다. 각 trial의 종료 후 진행 중인 요청을 비우는 시간도 처리율 분모에 포함했습니다.

아래 범위는 3회 trial의 최소~최대입니다. p50/p95는 각 trial 안의 전체 응답 완료 지연(ms)이며 trial 간 percentile을 합산하지 않았습니다. API/SSR 혼합이므로 개별 화면 지연으로 읽지 않습니다. 원본 JSON에는 경로별 지연도 남습니다.

| 공급자 조건 / 동시 요청 | 표본 n | 처리율 req/s | p50 ms | p95 ms | API/전송 오류 |
| --- | ---: | ---: | ---: | ---: | ---: |
| normal / 1 | 15,837 | 964.4–1106.1 | 0.8–0.9 | 1.7–2.1 | 0 |
| normal / 8 | 42,493 | 2805.7–2854.0 | 2.6–2.7 | 5.2–5.3 | 0 |
| normal / 32 | 38,616 | 2437.5–2750.1 | 13.0–14.0 | 19.2–23.6 | 0 |
| delay / 1 | 86 | 5.1–5.6 | 2.8–2.9 | 706.4–707.9 | 0 |
| delay / 8 | 705 | 41.3–41.3 | 5.9–6.5 | 713.0–713.9 | 0 |
| delay / 32 | 2,652 | 176.0–176.3 | 13.4–13.7 | 717.2–718.7 | 0 |
| failure / 1 | 17,624 | 1170.1–1180.0 | 0.7–0.7 | 1.7–1.7 | 0 |
| failure / 8 | 39,166 | 2438.0–2831.6 | 2.6–3.1 | 5.4–6.1 | 0 |
| failure / 32 | 36,299 | 2317.7–2483.3 | 13.2–14.7 | 22.4–23.7 | 0 |

| 조건 / 동시 요청 | Node CPU core-% | 최대 RSS MiB | pool 대기 최대 | DB wait 있는 표본/전체 | 공급자 호출/503 |
| --- | ---: | ---: | ---: | ---: | ---: |
| normal / 1 | 55.4–66.1 | 337.1 | 0 | 4/301 | 0/0 |
| normal / 8 | 108.4–113.1 | 318.0 | 4 | 17/300 | 0/0 |
| normal / 32 | 109.9–111.7 | 292.6 | 28 | 19/300 | 0/0 |
| delay / 1 | 2.2–5.0 | 293.0 | 0 | 0/323 | 23/0 |
| delay / 8 | 7.1–8.9 | 169.9 | 3 | 4/336 | 24/0 |
| delay / 32 | 14.1–16.5 | 182.5 | 24 | 2/296 | 21/0 |
| failure / 1 | 54.0–56.2 | 278.9 | 0 | 0/300 | 7/7 |
| failure / 8 | 107.5–108.9 | 290.6 | 4 | 8/300 | 8/8 |
| failure / 32 | 109.5–111.3 | 295.9 | 27 | 16/300 | 8/8 |

**193,478개** 완료 요청에서 API/전송 오류는 0건이었습니다. 이는 날씨 성공률이 아닙니다. failure 조건의 공급자 503은 총 23회이고 날씨 API는 `unavailable`을 반환했습니다. 정상은 priming 이후 cache hit로 공급자 추가 호출이 없었습니다. 지연은 TTL 0·700ms로 설정해 반복 miss를 만들었고, 동시 32에서는 3회 동안 공급자 호출 21회와 공유 합류 651회를 관측했습니다. failure의 backoff도 유지됐습니다. timeout은 별도의 전송 시나리오에서 보류/기한 초과로 검사했습니다.

CPU 100%는 논리 코어 1개 분량이며 Mac 전체 CPU 비율이 아닙니다. Caddy·PostgreSQL·부하 발생기의 전체 CPU/메모리는 이 표에 포함하지 않습니다. RSS는 seeding·앞선 실행·GC 상태를 포함한 최대 관측이고, DB wait는 짧은 대기를 놓칠 수 있는 50ms 표본입니다. 원본 JSON에는 20ms 해상도의 event loop p95도 기록했습니다. 정상 조건에서 동시 8→32로 늘려도 처리율이 증가하지 않았고 pool 대기 최대는 4→28로 늘었습니다. 이 사실만으로 DB 또는 Node 하나를 유일한 병목으로 확정하지 않습니다.

로그인·글 쓰기·모임 mutation 처리량, 고정 도착률의 과부하, 장시간 soak, 운영 서버 사양·네트워크·SLO는 이번 표에 없습니다. 따라서 **운영 동시 사용자 수/운영 수용량은 여전히 미검증**입니다. 공개 HTTPS 대상이 정해지면 같은 설정의 전송 검사와 별도 부하 발생기에서의 측정을 다시 실행합니다.

디버깅: 최초 검사기의 본문 링크 문자열과 HTTP/2 SNI/authority를 실제 주소에 맞췄고, 잘못된 Origin 검사는 올바른 JSON Content-Type과 함께 보내 403을 관측했습니다. 앞선 TLS 시험의 로그인과 부하 준비 로그인이 연속되어 발생한 429는 독립 시험 schema의 준비용 제한 기록만 초기화했습니다. 제품 제한은 완화하지 않았습니다. 프록시 환경의 IP가 모두 같은 값이 되는 문제는 `TRUST_PROXY=loopback`으로 지정한 peer만 신뢰하도록 보완했으며 기본 직접 실행은 기존대로 전달 헤더를 신뢰하지 않습니다.
