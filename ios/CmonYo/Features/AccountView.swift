    ...
    NavigationStack {
      List {
        ...
        if let user = session.user {
          Section("내 계정") { Text(user.name + "님"); Text(user.email) }
          ...
          Button("이메일로 로그인") { login = LoginRoute() }.disabled(session.restoring)
  ...
  var body: some View {
    NavigationStack {
      Form {
        ...
        }.disabled(pending || email.isEmpty || password.count < 12 || password.count > 128 || (signup && name.trimmingCharacters(in: .whitespaces).isEmpty))
