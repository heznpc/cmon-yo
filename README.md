...
**시설 지도 — 2026-09-14:** Web은 실제 지도와 시설 목록을 함께 표시하고, Native는 MapKit에 같은 공원 좌표를 핀으로 표시합니다. 허용된 기기 위치는 현재 위치 마커로 표시하며 권한 거부·타임아웃에도 시설 탐색을 유지합니다. 검색 → 핀 선택 → 시설 정보 → 상세·모임 이동을 연결했습니다. 지도 설정키는 추가로 필요하지 않습니다. [지도 연결과 실패 동작](docs/integrations.md#시설-지도)을 참고합니다.
