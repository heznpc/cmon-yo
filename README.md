...
**PR1 기능 수용조건 충족 / XCUITest 자동화 미검증**입니다. fixture 기반 Web·API·iOS의 필수 실행 증거는 아래에 기록했습니다. PR1에는 실제 모집·로그인·참여·댓글 쓰기를 포함하지 않았습니다. 현재 이메일 계정과 실제 모임의 진행 상태는 아래 최신 기록을 따릅니다. 최초 구현은 기본 UI였으며, 현재 화면·테마 변경은 아래 최신 기록을 따릅니다. 아래의 통과·미검증을 구분합니다.

**작성 복구·로컬 관측·인증 웹뷰 기준선 — 2026-09-14:** [PR #16](https://github.com/heznpc/cmon-yo/pull/16)의 `69c71d6`에서 초안 보존·미확인 요청 복구, 공통 명령 처리, 로컬 오류·요청 ID·캐시 관측, Native 웹 댓글 작성 후 같은 모임 복귀를 구현하고 실행 검증했습니다. 이번 작업은 검증한 코드의 커밋·푸시와 실행 방법·검증 범위 기록으로 종료합니다. [공개 기준선과 재현 방법](docs/release-baseline.md)을 따르며, 운영 모니터링·서비스 배포·사용자 모집과 추가 트래픽 대응은 조건부 후속입니다. 아래 이전 단계의 미검증 기록이 이번 종료 기준을 확대하지 않습니다.

PR1 기능 기준선은 main에 반영됐습니다. **PR2는 적재된 지역의 시설·날씨 연결을 구현했으며 기상청 인증 실응답 검증이 남아 [draft PR #2](https://github.com/heznpc/cmon-yo/pull/2)**입니다. 무안군 데이터는 회귀 표본이고 기본 지역이 아닙니다. [PR2 수용조건](docs/pr2-acceptance.md)과 아래 실행 기록을 따릅니다.
...
**시설 지도 — 2026-09-14:** Web은 실제 지도와 시설 목록을 함께 표시하고, Native는 MapKit에 같은 공원 좌표를 핀으로 표시합니다. 허용된 기기 위치는 현재 위치 마커로 표시하고 가까운 시설을 우선 조회하며, 사용자가 동네를 선택하면 해당 지역을 우선합니다. 권한 거부·타임아웃에도 시설 탐색을 유지합니다. 검색 → 핀 선택 → 시설 정보 → 상세·모임 이동을 연결했습니다. 지도 설정키는 추가로 필요하지 않습니다. [지도 연결과 실패 동작](docs/integrations.md#시설-지도)을 참고합니다.
...
현재 코드는 **Web·iOS 이메일 로그인과 실제 모임 생성·참여·취소·내 모임을 PostgreSQL에 연결한 상태**입니다. 주최 수정/취소, 동네·종목·날짜·시설별 탐색, 계정별 개인 조회·Native Keychain 복원을 구현했습니다. [이번 수용조건과 실행 증거](docs/meetups-acceptance.md)를 따릅니다. 공식 OAuth·외부 메일과 Native 소통 화면의 추가 확장은 후속이며, 인증된 WebView는 댓글 작성·복귀 한 흐름을 검증했습니다. [handoff §1.4~1.6](docs/CMON_YO_FINAL_HANDOFF.md#14-사용자-시나리오와-화면-연결--구현-기준선)에 사용자 시나리오·화면/API·외부 설정을, §2.1에 현재 코드 근거를, §16에 구현 순서와 실행 기준을 정리했습니다.
